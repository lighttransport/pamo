...
pamo_error pamo_dual_mc(pamo_mesh *out, const double *sdf, int32_t R,
    ...
    /* For each axis, iterate over grid edges with sign changes and
     * emit a quad from the 4 adjacent surface cells (ring order). */
        ...
        for (int32_t iy = 0; iy < R; iy++)
            for (int32_t ix = 0; ix < C; ix++) {
                ...
                if (nc == 4) {
                                   ...
                                   c[0], c[1], c[2], c[3], v0 < v1);
                } else if (nc == 3 && fi < max_faces) {
                    if (v0 < v1) out->faces[fi++] = (pamo_tri){{c[0], c[2], c[1]}};
                    else         out->faces[fi++] = (pamo_tri){{c[0], c[1], c[2]}};
                }
            }
